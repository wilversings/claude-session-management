...
// Get the title from already-read session content: a custom-title (/rename or
// star) wins over the auto-generated ai-title. The last matching record wins.
...
// Get the title of a session file (reads the file, then delegates).
function sessionTitle(file) {
    return titleFromContent(readText(file));
}

// The number of context-window tokens the session was last occupying: the
// input side of the most recent assistant turn (fresh input + cached prompt +
// newly cached prompt). This is the figure Claude Code shows on its context
// meter. Returns null when the session has no usage records (e.g. never ran).
//
// We scan lines from the end for the last one carrying a usage object and parse
// just that line — cheaper and more robust than JSON-parsing the whole file.
function contextTokensFromContent(content) {
...
function titleIsStarred(title) {
    return title.startsWith(STAR);
}

// The starred form of a title: ⭐ in front (an empty title becomes just ⭐).
function starTitle(title) {
    return title ? `${STAR} ${title}` : STAR;
}

// The unstarred form of a title: leading ⭐ (and the space after it) stripped.
function unstarTitle(title) {
    return titleIsStarred(title) ? title.slice(STAR.length).trimStart() : title;
}
...
// Format a token count compactly: '—' when unknown, '842' under 1k, else '128.4k'.
...
// Format a byte count as KB up to 1 MiB, then MB — e.g. '342 KB', '7.5 MB'.
